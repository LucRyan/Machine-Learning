...
function Destroyed()
{
	...
	if ( Squad != None )
		UTSquadAI(Squad).RemoveBot(class'MyBot');
		...
		if (Wall == RouteGoal || (Vehicle(RouteGoal) != None && Wall == Vehicle(RouteGoal).GetVehicleBase()))
		{
			if (LastBlockingVehicle != None)
			{
				...
				if (Vehicle(Pawn) != None)
				{
					...
					UTSquadAI(Squad).BotEnteredVehicle(class'MyBot');
					WhatToDoNext();
				}
			}
			return true;
...
{
	return (ImpactJumpZ > 0.f && Pawn.Health >= 80 && Skill >= 5.0 && UTSquadAI(Squad).AllowImpactJumpBy(class'MyBot'));
}
			...
			else if (UTWeap.bRecommendSplashDamage && Focus.Physics == PHYS_Falling && Skill + Accuracy + Tactics > 2.0 + 2.0 * FRand())
			{
				if (UTWeap.CurrentFireMode < UTWeap.WeaponProjectiles.length && UTWeap.WeaponProjectiles[UTWeap.CurrentFireMode] != None)
				{
					...
					if (Trace(HitLocation, HitNormal, Focus.Location - vect(0,0,10000), Focus.Location, false, Pawn(Focus).GetCollisionExtent()) != None)
					{
						...
						ProjTime = UTWeap.WeaponProjectiles[UTWeap.CurrentFireMode].static.StaticGetTimeToLocation(TargetLoc, UTWeap.GetPhysicalFireStartLoc(), class'MyBot');
...
function SendMessage(PlayerReplicationInfo Recipient, name MessageType, float Wait, optional class<DamageType> DamageType)
{
	...
	UTPlayerReplicationInfo(PlayerReplicationInfo).VoiceClass.static.SendVoiceMessage(class'MyBot', Recipient, MessageType, DamageType);
}
...
function SetBotOrders(name NewOrders, Controller OrderGiver, bool bShouldAck)
{
	...
	UTTeamInfo(PlayerReplicationInfo.Team).AI.SetOrders(class'MyBot',NewOrders,OrderGiver);
	WhatToDoNext();
}
...
function ClearTemporaryOrders()
{
	if (OldOrders != 'None')
	{
		...
		UTTeamInfo(PlayerReplicationInfo.Team).AI.SetOrders(class'MyBot',OldOrders,OldOrderGiver);
...
event HearNoise(float Loudness, Actor NoiseMaker, optional Name NoiseType )
{
	...
	if ( UTSquadAI(Squad).SetEnemy(class'MyBot', NoiseMaker.Instigator))
	{
		WhatToDoNext();
	}
}
...
{
	if (Squad == None && !WorldInfo.GRI.OnSameTeam(class'MyBot', SeenPlayer))
	{
		// maybe scripted pawn; just notify Kismet
		Pawn.TriggerEventClass(class'SeqEvent_AISeeEnemy', SeenPlayer);
	}
	else if (UTSquadAI(Squad).SetEnemy(class'MyBot', SeenPlayer))
	{
...
function bool AssignSquadResponsibility()
{
	...
	return UTSquadAI(Squad).AssignSquadResponsibility(class'MyBot');
}
...
event MayDodgeToMoveTarget()
{
	...
	Dir = MoveTarget.GetDestination(class'MyBot') - Pawn.Location;
	Dist = VSize(Dir);
...
function Possess(Pawn aPawn, bool bVehicleTransition)
{
	if (aPawn.bDeleteMe)
	{
		`Warn(class'MyBot' @ GetHumanReadableName() @ "attempted to possess destroyed Pawn" @ aPawn);
		ScriptTrace();
		GotoState('Dead');
	}
	else
...
event WhatToDoNext()
{
	...
	{
		if (!bSpawnedByKismet && UTGame(WorldInfo.Game).TooManyBots(class'MyBot'))
		{
			if ( Pawn != None )
			{
				...
				{
					Pawn.Health = 0;
					Pawn.Died( class'MyBot', class'DmgType_Suicided', Pawn.Location );
				}
			}
			Destroy();
			return;
...
protected event ExecuteWhatToDoNext()
{
	...
	{
		UTTeamInfo(PlayerReplicationInfo.Team).SetBotOrders(class'MyBot');
	}
	SwitchToBestWeapon();
	...
	if ( (Enemy != None) && ((Enemy.Health <= 0) || (Enemy.Controller == None)) )
		LoseEnemy();
	if ( Enemy == None )
		UTSquadAI(Squad).FindNewEnemyFor(class'MyBot',false);
	else if ( !UTSquadAI(Squad).MustKeepEnemy(Enemy) && !LineOfSightTo(Enemy) )
	{
		// decide if should lose enemy
		if ( UTSquadAI(Squad).IsDefending(class'MyBot') )
		{
			if ( LostContact(4) )
				LoseEnemy();
		}
...
function VehicleFightEnemy(bool bCanCharge, float EnemyStrength)
{
	...
	if ( !LineOfSightTo(Enemy) )
	{
		...
		if ( !bCanCharge || (UTSquadAI(Squad).IsDefending(class'MyBot') && LostContact(4)) )
		{
			GoalString = "Stake Out";
			DoStakeOut();
		}
	...
	if ( V.RecommendCharge(class'MyBot', Enemy) )
	{
		GoalString = "Charge";
		DoCharge();
		return;
...
function FightEnemy(bool bCanCharge, float EnemyStrength)
{
	...
	if (Squad != None)
	{
		UTSquadAI(Squad).ModifyAggression(class'MyBot', Aggression);
	}
	...
	if (!Pawn.CanAttack(Enemy))
	{
		...
		else if ( UTSquadAI(Squad).IsDefending(class'MyBot') && LostContact(4) && ClearShot(LastSeenPos, false) )
		{
		...
		else if ( (((Aggression < 1) && !LostContact(3+2*FRand())) || IsSniping()) && CanStakeOut() )
		{
			...
			(Squad == None || !UTSquadAI(Squad).HasOtherVisibleEnemy(class'MyBot')) )
		{
...
function ChooseAttackMode()
{
	...
	if ( (UTSquadAI(Squad).PriorityObjective(class'MyBot') == 0) && (Skill + Tactics > 2) && ((EnemyStrength > -0.3) || (Pawn.Weapon.AIRating < 0.5)) )
	{
		if ( Pawn.Weapon.AIRating < 0.5 )
		{
...
function bool FindSuperPickup(float MaxDist)
{
	...
	if ( (UTDeathMatch(WorldInfo.Game) != None) && !UTDeathMatch(WorldInfo.Game).WantsPickups(class'MyBot') )
		return false;
	...
	if ( BestPath != None )
	{
		...
		{
			P.TeamOwner[PlayerReplicationInfo.Team.TeamIndex] = class'MyBot';
...
event bool NotifyBump(Actor Other, Vector HitNormal)
{
	...
	if ( (P == None) || (P.Controller == None) || (Enemy == P) )
		return false;
	if ( (Squad != None) && UTSquadAI(Squad).SetEnemy(class'MyBot',P) )
	{
		WhatToDoNext();
		return false;
	}
...
{
	return (UTSquadAI(Squad).PriorityObjective(class'MyBot') > 0);
}
...
function SetFall()
{
	...
	{
		bPlannedJump = true;
		AdjustedDest = (MoveTarget != None) ? MoveTarget.GetDestination(class'MyBot') : GetDestinationPosition();
		Pawn.SuggestJumpVelocity(Pawn.Velocity, (MoveTarget != None) ? MoveTarget.GetDestination(class'MyBot') : GetDestinationPosition(), Pawn.Location);
...
{
	return UTSquadAI(Squad).AllowDetourTo(class'MyBot',N);
}
...
event DelayedWarning()
{
	...
	{
		if ( Squad != None )
			UTSquadAI(Squad).SetEnemy(class'MyBot', WarningProjectile.Instigator);
		return;
	}
...
{
	LastUnderFire = WorldInfo.TimeSeconds;
	if ( (instigatedBy != None) && (instigatedBy != class'MyBot') )
...
function DelayedInstantWarning()
{
	...
	if ( Enemy == None )
	{
		if ( Squad != None )
		{
			UTSquadAI(Squad).SetEnemy(class'MyBot', InstantWarningShooter);
		}
		return;
	}
...
event ReceiveWarning(Pawn shooter, float projSpeed, vector FireDir)
{
	...
	{
		if ( Squad != None )
			UTSquadAI(Squad).SetEnemy(class'MyBot', shooter);
		return;
	}
...
{
	if ( !WorldInfo.GRI.OnSameTeam(V,class'MyBot') )
	{
...
function Actor FaceActor(float StrafingModifier)
{
	...
	SquadFace = UTSquadAI(Squad).SetFacingActor(class'MyBot');
	if ( SquadFace != None )
		return SquadFace;
	...
	if ( RelativeDir > 0.85 )
		return Enemy;
	if ( (RelativeDir > 0.3) && (MyBot(Enemy.Controller) != None) && (MoveTarget == Enemy.Controller.MoveTarget) )
		return Enemy;
...
event float RatePickup(Actor PickupHolder, class<Inventory> InvClass)
{
	...
	{
		return UTWeaponLocker(PickupHolder).BotDesireability(bCheckDriverPickups ? Vehicle(Pawn).Driver : Pawn, class'MyBot');
	}
	return InvClass.static.BotDesireability(PickupHolder, bCheckDriverPickups ? Vehicle(Pawn).Driver : Pawn, class'MyBot');
}
...
event float SuperDesireability(PickupFactory P)
{
	if ( !SuperPickupNotSpokenFor(UTPickupFactory(P)) )
		return 0;
	return P.InventoryType.static.BotDesireability(P, bCheckDriverPickups ? Vehicle(Pawn).Driver : Pawn, class'MyBot');
}

function bool SuperPickupNotSpokenFor(UTPickupFactory P)
{
	local MyBot CurrentOwner;
	...
	CurrentOwner = MyBot(P.TeamOwner[PlayerReplicationInfo.Team.TeamIndex]);

	if ( (CurrentOwner == None ) || (CurrentOwner == class'MyBot')
...
{
	if ( (Pawn.health > 0) && (Damage > 0) && (Other.Pawn != None) && (Squad != None) && UTSquadAI(Squad).SetEnemy(class'MyBot',Other.Pawn) )
		WhatToDoNext();
}
...
function OnAIFreeze(UTSeqAct_AIFreeze FreezeAction)
{
	...
	{
		FreezeAction.ScriptLog(class'MyBot' @ "received unfreeze request while not frozen");
	}
}
...
state Defending
{
	...
	function SuggestDefenseRotation()
	{
		...
		if (DefenseAnchor.PathList.length > 0)
		{
			...
			FormationCenter = UTSquadAI(Squad).FormationCenter(class'MyBot');
	...
	function SetRouteGoal()
	{
		...
		if (DefensePoint == None || UTPlayerReplicationInfo(PlayerReplicationInfo).bHasFlag )
		{
			...
			if ( (WorldInfo.TimeSeconds - FMax(LastSeenTime, AcquireTime) < 5.0 || FRand() < 0.85)
				...
				&& UTSquad.AcceptableDefensivePosition(DefensivePosition, class'MyBot') )
			{
				CampTime = 3;
				GotoState('Defending','Pausing');
			}
			DefensivePosition = UTSquad.FindDefensivePositionFor(class'MyBot');
			if ( DefensivePosition == None )
			{
				RouteGoal = UTSquad.FormationCenter(class'MyBot');
			}
			else
			{
	...
	function BeginState(Name PreviousStateName)
	{
		...
		else if ( (UTSquad.SquadLeader != class'MyBot') && (UTSquad.SquadLeader.Pawn != None) && (UTSquad.FormationCenter(class'MyBot') == UTSquad.SquadLeader.Pawn) )
...
state MoveToGoal
{
	function bool CheckPathToGoalAround(Pawn P)
	{
		...
		PBot = MyBot(P.Controller);
		if ( (MoveTarget == None) || (PBot == None) || !WorldInfo.GRI.OnSameTeam(class'MyBot',P.Controller) )
			return false;

		PSquad = UTSquadAI(PBot.Squad);
		return ( (PSquad != None) && PSquad.ClearPathFor(class'MyBot') );
	}
...
Begin:
	SwitchToBestWeapon();
	WaitForLanding();
	if ( Pawn.bCanPickupInventory && (UTPickupFactory(MoveTarget) != None) && (UTSquadAI(Squad).PriorityObjective(class'MyBot') == 0) && (Vehicle(Pawn) == None)
		&& UTPickupFactory(MoveTarget).ShouldCamp(class'MyBot', 5) )
	{
...
state Fallback extends MoveToGoalWithEnemy
{
	function bool FireWeaponAt(Actor A)
	{
		if ( (A == Enemy) && (Pawn.Weapon != None) && (Pawn.Weapon.AIRating < 0.5)
			...
			&& (UTSquadAI(Squad).PriorityObjective(class'MyBot') == 0)
	...
	event bool NotifyBump(Actor Other, Vector HitNormal)
	{
		...
		if ( (P == None) || (P.Controller == None) )
			return false;
		if ( (RouteCache.Length > 0) && !WorldInfo.GRI.OnSameTeam(class'MyBot',P.Controller) && (MoveTarget == RouteCache[0]) && (RouteCache.Length > 1) && P.ReachedDestination(MoveTarget) )
		{
			MoveTimer = VSize(RouteCache[1].Location - Pawn.Location)/(Pawn.GroundSpeed * Pawn.DesiredSpeed) + 1;
			MoveTarget = RouteCache[1];
		}
		UTSquadAI(Squad).SetEnemy(class'MyBot',P);
		if ( Enemy == Other )
		{
			Focus = Enemy;
			TimedFireWeaponAtEnemy();
	...
	{
		if ( UTSquadAI(Squad).FindNewEnemyFor(class'MyBot',false) || (Enemy == None) )
			WhatToDoNext();
		else
		{
			enable('SeePlayer');
...
function bool LoseEnemy()
{
	...
	if ( UTSquadAI(Squad).LostEnemy(class'MyBot') )
	{
		bFrustrated = false;
		return true;
	}
...
function DoRetreat()
{
	if ( UTSquadAI(Squad).PickRetreatDestination(class'MyBot') )
	{
...
ignores SeePlayer, HearNoise;
	...
	*/
	function PickDestination()
	{
		...
		if ( Pawn == None )
		{
			`warn(class'MyBot'$" Tactical move pick destination with no pawn");
			return;
		}
		bChangeDir = false;
...
ignores EnemyNotVisible;
	...
	*/
	...
	function PickDestination()
	{
		...
		if (!bDirectHunt)
		{
			UTSquadAI(Squad).MarkHuntingSpots(class'MyBot');
		}
...
Begin:
	...
	if ( FocusOnLeader(false) )
		FireWeaponAt(Focus);
	else if ( (Pawn.Weapon != None) && !Pawn.Weapon.bMeleeWeapon && UTSquadAI(Squad).ShouldSuppressEnemy(class'MyBot') && ClearShot(GetFocalPoint(),true) )
	{
...
ignores SeePlayer, HearNoise, Bump;
	...
	{
		if (Focus == KilledPawn && Killed != class'MyBot')
		{
			WhatToDoNext();
		}
	...
	function bool FindStrafeDest()
	{
		...
		else if (Pawn.Anchor.PathList.length > 0)
		{
			...
			if (!WorldInfo.GRI.OnSameTeam(Focus, class'MyBot'))
			{
...
ignores SeePlayer, EnemyNotVisible, HearNoise, ReceiveWarning, NotifyLanded, NotifyPhysicsVolumeChange,
	...
	{
		//`log(class'MyBot' @ "WhatToDoNext while dead);
		//ScriptTrace();
	}

	function Celebrate()
	{
		`log(class'MyBot'$" Celebrate while dead");
	}

	function bool SetRouteToGoal(Actor A)
	{
		`log(class'MyBot'$" SetRouteToGoal while dead");
		return true;
	}

	function SetAttractionState()
	{
		`log(class'MyBot'$" SetAttractionState while dead");
	}

	function EnemyChanged(bool bNewEnemyVisible)
	{
		`log(class'MyBot'$" EnemyChanged while dead");
	}

	function WanderOrCamp()
	{
		`log(class'MyBot'$" WanderOrCamp while dead");
	}
...
TryAgain:
	...
	{
		Sleep(0.75 + UTGame(WorldInfo.Game).SpawnWait(class'MyBot'));
		LastRespawnTime = WorldInfo.TimeSeconds;
		WorldInfo.Game.ReStartPlayer(class'MyBot');
		Goto('TryAgain');
	}
...
ignores SeePlayer, HearNoise, KilledBy, NotifyBump, HitWall, NotifyPhysicsVolumeChange, NotifyHeadVolumeChange,
	...
	function WhatToDoNext()
	{
		`Log(class'MyBot' @ "WhatToDoNext while RoundEnded");
		// ScriptTrace();
	}

	function Celebrate()
	{
		`log(class'MyBot'$" Celebrate while RoundEnded");
	}

	function SetAttractionState()
	{
		`log(class'MyBot'$" SetAttractionState while RoundEnded");
	}

	function EnemyChanged(bool bNewEnemyVisible)
	{
		`log(class'MyBot'$" EnemyChanged while RoundEnded");
	}

	function WanderOrCamp()
	{
		`log(class'MyBot'$" WanderOrCamp while RoundEnded");
	}
 ...
 */
delegate bool CustomActionFunc(MyBot B)
{
	return true;
}
...
Begin:
	while (!CustomActionFunc(class'MyBot'))
	{
		Sleep(0.0);
	}
...
event bool HandlePathObstruction(Actor BlockedBy)
{
	...
	if (!Pawn.IsA('Vehicle'))
	{
		...
		if (V != None && Squad != None && UTSquadAI(Squad).VehicleDesireability(V, class'MyBot') > 0.0)
		{
	...
	P = Pawn(BlockedBy);
	if (P != None && !WorldInfo.GRI.OnSameTeam(class'MyBot', P))
	{
	...
	// ask Squad
	return (Squad != None) ? UTSquadAI(Squad).HandlePathObstruction(class'MyBot', BlockedBy) : false;
}
...
state Frozen
{
	...
	function OnAIFreeze(UTSeqAct_AIFreeze FreezeAction)
	{
		...
		{
			FreezeAction.ScriptLog(class'MyBot' @ "received freeze request while already frozen");
		}
	}
